...
- **Daily P&L cap incl. unrealized + cap-flatten (S204, 2026-06-04):** the cap (`e2t_daily_pnl_cap`) gates new entries on realized + open unrealized, AND when realized+unrealized ≥ cap on 2 consecutive 5s checks the main loop flattens all slots, sets persisted `cap_hit_today` (blocks rest of day), sends 🎯 Telegram. Kill switch `cap_flatten_enabled` (default true). Cap policy: cap = account's best day − ~$25 buffer (E2T 30% consistency rule — profit over cap raises required total $3.33:1). LONG $740 / SHORT $525.
