...
# Exchange + regulatory fees per contract per SIDE. The TS order API reports
# ONLY its own `CommissionFee` ($0.50/side, verified exact on 41 filled orders /
# 68 contract-sides, 2026-08-10..14) and `UnbundledRouteFee` (always 0) — CME
# exchange + NFA fees appear nowhere in the API, only in account equity.
# Measured 2026-08-15 from the equity gap over that week: $21.12 / 22 sides on
# 210VYX65 and $44.16 / 46 sides on 210VYX91 = $0.96/side all-in in BOTH
# accounts, to the cent. $0.96 - $0.50 commission = $0.46/side of fees.
# All-in cost is therefore $1.92 per contract round-turn.
FEE_PER_SIDE = float(os.getenv("MES_FEE_PER_SIDE", "0.46"))
    ...
    for acct in ACCOUNTS:
        ...
        for o in orders:
            if o.get("Status") not in ("FLL", "FLP"):
                continue
            # CommissionFee is per ORDER, so split it across legs by exec qty
            # (single-leg in practice, but this stops a multi-leg order from
            # counting the whole fee once per leg). Exchange + regulatory fees
            # are absent from the API and are added per contract-side.
            comm = float(o.get("CommissionFee") or 0)
            order_qty = sum(int(l.get("ExecQuantity") or 0) for l in o.get("Legs", []))
            for leg in o.get("Legs", []):
                ...
                fills.append({
                    ...
                    "commission": (comm * q / order_qty if order_qty else comm)
                                  + q * FEE_PER_SIDE,
                })
...
<div class='src'>Source: TradeStation /brokerage/accounts/{{{', '.join(ACCOUNTS)}}}/historicalorders, FIFO round-trip matching.
Costs: ${0.50:.2f}/side TS commission (from the API) + ${FEE_PER_SIDE:.2f}/side exchange &amp; regulatory fees
(not reported by the API; measured from account equity 2026-08-15) = <b>${(0.50 + FEE_PER_SIDE) * 2:.2f} per contract round-turn</b>.
