...
**V11 filter (current):** V10 + time-of-day gates + SC grade gate. SC/DD blocked 14:30-15:00 ET (charm dead zone, 35% WR). SC/DD blocked 15:30+ ET (too little time). BofA blocked after 14:30 ET (0% WR). **SC grade gate: only A+/A/B pass (C and LOG blocked).** V2 grading backtest: A+=87% WR, A=79%, B=72%, C=52%, LOG=24%. Blocking C/LOG costs -$0.9 PnL but cuts MaxDD 37%. Grade gate is SC-only — other setups not analyzed yet. V10 base: Longs alignment >= +2 AND (VIX <= 22 OR overvix >= +2), SC exempt from VIX gate. Shorts whitelist: Skew Charm (all), AG Short (all), DD Exhaustion (align!=0). **SC/DD shorts blocked when paradigm = GEX-LIS** (24t, 43% WR — LIS acts as support floor). GEX-PURE (81% WR) and GEX-MESSY (83% WR) NOT blocked. Overvix = VIX - VIX3M; when >= +2 means market overvixed. `_passes_live_filter()` accepts `paradigm` and `grade` params, all 5 callers pass both. Filter history: R1 → V7 → V7+AG → V8 → V9-SC → V10 → **V11**.
