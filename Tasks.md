...
| S39 | **Deploy cap=2 for SHORTS account (weekend)** | 2026-04-18 Saturday or Sunday (before Mon 09:30 ET) | Raise `MAX_CONCURRENT_PER_DIR` for shorts account (210VYX91) from 1 → 2. LONGS account (210VYX65) stays at 1. Study: TSRT-live slot analysis showed 12 missed shorts worth +244 pts (91% WR) over 17 days. Scenario 5 (V13 + 2-slot) projects +$1,842/mo on 1 MES vs current +$1,096/mo at 1-slot. Implementation: edit `real_trader.py` `MAX_CONCURRENT_PER_DIR`, test in SIM or paper first, deploy outside market hours. Margin: each short slot ~$700, so account needs ~$1,400 available (currently ~$1,558 — fits). See TSRT_Scenarios_Reconciled.html in Tel Res msg 20. | PENDING — SAT/SUN THIS WEEKEND |
