    ...
    if (absDelta >= threshold)
    {
        ...
        if (direction != 0 && idx > LastSignalIdx)
        {
            ...
            if (SignalPath.GetLength() > 0)
            {
                ...
                // sc.OpenFile() returns 1 on success / 0 on failure, so the prior
                // `== 0` check was inverted (only ran on failure → silent zero-byte file).
                // Switch to standard C fopen for reliability (same pattern as VXDomSnapshot).
                FILE* f = fopen(SignalPath.GetChars(), "wb");
                if (f != NULL)
                {
