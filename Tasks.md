...
| S17 | **VPS setup completion** | 2026-03-29 (next session) | Kamatera VPS (103.54.56.210). **Installed:** Python 3.12.9, Git 2.53.0, pip 24.3.1, VS Code 1.113.0, GitHub CLI (logged in), Railway CLI (logged in), Sierra Chart, Google Drive, Git Bash, NT8+Rithmic, eval_trader+watchdog+auto-start. **Remaining:** Test trade (`--test buy`), Railway CLI link, DTC server config, ES+VX symbols, S18 data bridge. | IN PROGRESS |
...
| S23 | **Deploy stale-order cleanup fix** | 2026-04-06 after 16:10 ET | Bug: #1540 (Apr 2, unfilled) stayed in `_active_orders` over weekend, blocked #1544/#1551 shorts via MAX_CONCURRENT=1. Fix: (1) `cleanup_stale_orders()` runs daily 09:28 ET, (2) cap-skip log shows blocking order details. Files: `app/real_trader.py`, `app/main.py`. Commit+push+deploy after market close. | PENDING |
