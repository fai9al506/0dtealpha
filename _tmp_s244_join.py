...
Writes _tmp_s244_trades.pkl
"""
import os, pickle, bisect
import psycopg2

MAX_LAG_S = 240  # 2-min cadence -> accept nearest snapshot within 4 min
...
def main():
    ...
    for (lid, ts, name, direction, spot, pnl, res, mespnl, para, grade, vix, lp, mfe) in rows:
        ...
        cands = [j for j in (i - 1, i) if 0 <= j < len(ep)]
        if not cands:
            miss += 1; continue
        j = min(cands, key=lambda k: abs(ep[k] - e))
        if abs(ep[j] - e) > MAX_LAG_S:
            miss += 1; continue
        s = snaps[j]
        spot = float(spot)
    ...
    with open("_tmp_s244_trades.pkl", "wb") as fh:
        pickle.dump(out, fh)
