"""Is `align` the discriminator for gate-removed GEX Long losers (the 11:22-at-the-top
case)? Generate ALL ABC-structure signals (TS GEX, NO align pre-filter), split by
align value AND by upside-to-magnet at entry (the 'fired at the top' mechanism)."""
...
def simulate(ts, entry, target):
    ...
    sl=entry-SL; mfe=0; mae=0; ta=False; tstop=sl
    for sp in path:
        mfe=max(mfe,sp-entry); mae=min(mae,sp-entry)
        stop=tstop if ta else sl
        if sp<=stop: return (('WIN' if stop-entry>0 else 'LOSS'),stop-entry,mfe,mae)
        if sp>=target: return ('WIN',target-entry,mfe,mae)
        if not ta and mfe>=TACT: ta=True; tstop=entry+(mfe-TGAP)
        elif ta:
            nt=entry+(mfe-TGAP)
            if nt>tstop: tstop=nt
    return ('EXPIRED',path[-1]-entry,mfe,mae)
...
snaps=cur.fetchall()
print(f"chain snapshots: {len(snaps)}")
...
for ts,t,spot,rows in snaps:
    ...
    v=classify(f)
    if v not in ('A++','A','B'): continue   # NO align filter here
    ...
    upside=(mpg-spot) if mpg else 0   # 'fired at the top' = small/neg upside to magnet
    tgt=max(mpg or 0, spot+TFLOOR)
    sim=simulate(ts,spot,tgt)
    if not sim: continue
    last_fire[d]=t
    res,pnl,mfe,mae=sim
    sigs.append({'align':align,'upside':upside,'res':res,'pnl':pnl,'mfe':mfe,'mae':mae})

def st(g,label):
    if not g: print(f"{label:26s} n=0"); return
    n=len(g); w=sum(1 for s in g if s['res']=='WIN'); p=sum(s['pnl'] for s in g)
    print(f"{label:26s} n={n:3d}  WR={w/n*100:4.0f}%  PnL={p:+7.1f}p  avg={p/n:+5.1f}  (~${p*5:+,.0f})")

print(f"total ABC signals (no align filter): {len(sigs)}\n")
print("=== BY ALIGN VALUE ===")
for a in [3,2,1,0,-1,-2,-3]:
    st([s for s in sigs if s['align']==a], f"  align = {a:+d}")
print("-"*70)
st([s for s in sigs if s['align']>=0], "align >= 0")
st([s for s in sigs if s['align']<0],  "align <  0")
print("\n=== BY UPSIDE-TO-MAGNET AT ENTRY (the 'fired at the top' angle) ===")
for lo,hi,lab in [(-99,0,'<=0 (AT/above magnet)'),(0,5,'0-5pt'),(5,10,'5-10pt'),(10,20,'10-20pt'),(20,99,'>20pt')]:
    st([s for s in sigs if lo<s['upside']<=hi], f"  upside {lab}")
print("\n=== ALIGN x UPSIDE crosstab (WR%) ===")
for a in ['>=0','<0']:
    row=[s for s in sigs if (s['align']>=0 if a=='>=0' else s['align']<0)]
    near=[s for s in row if s['upside']<=5]; far=[s for s in row if s['upside']>5]
    def wr(g): return f"{sum(1 for s in g if s['res']=='WIN')/len(g)*100:.0f}%({len(g)})" if g else "-"
    print(f"  align {a:>3s}:  upside<=5: {wr(near):12s}  upside>5: {wr(far):12s}")
conn.close()
